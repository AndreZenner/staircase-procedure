...
public class StaircaseProcedure : MonoBehaviour
{
    ...
    public static StaircaseProcedure SP;
    ...
    [Tooltip("The socket is bound to the specified port (default: 65000)")]
    [SerializeField]
    ...
    /// <param name="numberOfSteps"> total number of steps from `minimum` to `maximum` (`minimum` will correspond to step `0`, `maximum` will correspond to step `numberOfSteps`) </param>
    ...
    public void Init(float minimumValue, float maximumValue, int numberOfSteps,
                    int startStepSequ1, int startStepSequ2, int stopAmount, int numberThresholdPoints,
                    string experimentName, string conditionName, int numberParticipant,
                    bool stopCriterionReversals = true, bool strictLimits = false, string plotTitle = "")
    {
        ...
        init = new InitData(minimumValue, maximumValue, numberOfSteps,
                     startStepSequ1, startStepSequ2, stopAmount, numberThresholdPoints,
                     resultsPath, experimentName, conditionName, numberParticipant,
                     stopCriterionReversals, strictLimits, plotTitle);
    ...
    void Awake()
    {
        Debug.Log("Hello! This is Staircase Procedure Toolkit v12");
        if (SP != null)
